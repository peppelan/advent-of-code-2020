input_ex = """light red bags contain 1 bright white bag, 2 muted yellow bags.
...
input_text = """dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
...
def sol(input_text):
    colors_to_containing_colors = {}
    
    for rule in input_text.split("\n"):
        # print("------", rule)
        container_color, contains = rule.split(" bags contain ")
        # print(container_color, " ||| ", contains)
        
        if "no other" in contains:
            continue
            
        for contain in contains.split(", "):
            number = int(contain.split(" ")[0])
            contained_color = " ".join(contain.split(" ")[1:-1])
            
            # print(container_color, number, contained_color)
            
            to_update = colors_to_containing_colors.get(contained_color, [])
            to_update.append(container_color)
            colors_to_containing_colors[contained_color] = to_update
            
    solving_bags = set()
    traverse_list = ['shiny gold']
    
    # DFS upwards
    while traverse_list:
        color = traverse_list.pop()
        
        if color not in colors_to_containing_colors:
            continue
            
        for next_color in colors_to_containing_colors[color]:
            if next_color not in solving_bags:
                solving_bags.add(next_color)
                traverse_list.append(next_color)
                
    return len(solving_bags)
