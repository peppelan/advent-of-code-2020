nput_ex = """shiny gold bags contain 2 dark red bags.
...
def sol(input_text):
    ...
    for rule in input_text.split("\n"):
        ...
        for contain in contains.split(", "):
            ...
            to_update = colors_to_contained_colors.get(container_color, [])
            to_update.append((number, contained_color))
            colors_to_contained_colors[container_color] = to_update
            
    solving_bags = 0
    traverse_list = [(1, 'shiny gold')]
    
    # DFS downwards
    while traverse_list:
        num, color = traverse_list.pop()
        
        if color not in colors_to_contained_colors:
            continue
            
        for next_num, next_color in colors_to_contained_colors[color]:
            this_qty = num * next_num
            solving_bags += this_qty
            traverse_list.append((this_qty, next_color))
