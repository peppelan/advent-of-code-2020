def calculateNoPars(toEval):
    ret = 1
    
    for additionsChunk in toEval.split(" * "):
        partSum = 0
        
        for addOperand in additionsChunk.split(" + "):
            partSum += int(addOperand)
...
def calculate(expr):
    while "(" in expr:
        ...
        for i, c in enumerate(expr):
            ...
            if c == ")":
                toEval = expr[last_open+1:i]
                expr = expr.replace("("+ toEval + ")", str(calculateNoPars(toEval)))
                break
        # print(toEval)
    return calculateNoPars(expr)

def sol(input_text):
    ret = 0
    for line in input_text.split("\n"):
        part_ret =  calculate(line)
        # print(part_ret)
        ret += part_ret
